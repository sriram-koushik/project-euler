...
class E41 {
    ...
    public static void main(String[] args)
    {
        ...
        /* We initialize the longestPDP to be -1 (clearly not a PDP) in case pan digital primes never exists
         * We also know that the largest possible Pan Digital number is 987654321 which fits in the INT range
         * Hence longestPDP is an integer number and we don't have to check anything else
         */
        int longestPDP = -1;
        try 
        {
            //Create a list of all digits 1-9
            final List<Integer> digits = new ArrayList<Integer>(Arrays.asList(9,8,7,6,5,4,3,2,1));
            /*
             * We are going to create all the permutations of lengths from 9 to 1,
             * from the 'digits' list.
             * We are starting from 9 because we need the longest pan digital number, so we can stop searching when we get one
             */
            for(int length = 9;length>=1;length--)
            {
                ...
                /*
                 * permutations() is called which will create a list of permutations for a
                 * a sublist of a given size.
                 * digits.subList(digits.size()-length, digits.size()) is a very cool hack:
                 * The digits is already ordered in non-increasing way. So for every iteration in the loop,
                 * we send a sublist after removing the digits.size()-length elements from the front
                 */
                permutations(perms, new ArrayList<Integer>(), digits.subList(digits.size()-length, digits.size()));
                //'perms' now contain all pan-digital permutations of size length
                for(int perm:perms)
                {
            ...
            if(longestPDP!=-1)
                System.out.println("The longest pan digital number is "+longestPDP);
            else
                System.out.println("There's no pandigital numbers which are prime");
        } 
        catch (Exception e) 
        {
